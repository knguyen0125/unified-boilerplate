...
function isGraphQlRequest(req: any): req is { req: { method: string } } {
  return (
    req && 'req' in req && typeof req.req === 'object' && 'method' in req.req
  );
}
// eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export class JoiPipe implements PipeTransform {
  ...
  private getTranslations() {
    if (this.translationCache.length) {
    ...
    this.translationCache = Object.fromEntries(
      ...
      if (Joi.isError(error)) {
        ...
        if (usePipeValidationException) {
          ...
          throw new BadRequestException({
            message: skipErrorFormatting ? error : formattedMessage,
            statusCode: 400,
  ...
  private parseOptions(pipeOpts?: JoiPipeOptions): ValidatedJoiPipeOptions {
    ...
    pipeOpts.defaultValidationOptions = Object.assign<
    ...
    const unknownKeys = Object.keys(pipeOpts).filter(
      (k) => !JOI_PIPE_OPTS_KEYS.includes(k),
    );
  ...
  private getSchema(metadata: ArgumentMetadata): Joi.Schema | undefined {
    ...
    if (this.type) {
      ...
      return JoiPipe.getTypeSchema(this.type, {
        forced: true,
        group: this.pipeOpts.group,
      });
    }

    // Determine the schema from the passed model
    if (metadata.metatype) {
      return JoiPipe.getTypeSchema(metadata.metatype, {
        group: this.pipeOpts.group,
      });
    }
  ...
  private static readonly typeSchemaMap = new Map<
    unknown,
    Map<string, Joi.Schema | undefined>
  >();

  /**
   * Obtain the type schema from getTypeSchema(). The result is cached for
   * better performance, using the type and options to construct a cache key.
   *
   * If no properties have been decorated in the whole chain, no schema will be
   * returned so as to not throw when inadvertently "validating" using inbuilt
   * types. Returning a schema can be forced to cover cases when a type has been
   * explicitely passed to JoiPipe.
   *
   * @param type The type (decorated class constructor) to construct a schema
   *   from
   * @param options An optional options object
   * @param options.forced Force an empty schema to be returned even if no
   *   properties of the type have been decorated. (defaults to false)
   * @param options.group An optional JoiValidationGroup to use during schema
   *   construction
   */
  private static getTypeSchema(
    ...
    const cacheKey =
      'forced' + (forced ? '1' : '0') + (group ? 'group' + String(group) : '');
    // Check cache.
    ...
    if (
      (!typeSchema || !Object.keys(typeSchema.describe().keys).length) &&
      !forced
    ) {
      // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
